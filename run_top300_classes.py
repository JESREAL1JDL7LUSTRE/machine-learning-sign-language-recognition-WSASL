"""
run_top300_classes.py — Full pipeline for the top-300 classes (~2828 videos)
=============================================================================
Runs: extract -> normalize -> resample -> train (all 3 models) -> compare results
"""
...
TOP_CLASSES = [c for c, count in sorted_classes[:300]]
...
for idx, cls in enumerate(TOP_CLASSES):
    ...
    print(f"[{idx+1:3d}/{len(TOP_CLASSES)}] '{cls:<15}' — {len(videos):>2d} videos", end="", flush=True)
    ok = 0
    for vf in videos:
...
# ─────────────────────────────────────────────────────────────────────────────
# STEP 3 — TRAIN & COMPARE ALL 3 MODELS
# ─────────────────────────────────────────────────────────────────────────────
print("\n" + "="*60)
print("STEP 3/4 — Training & comparing all 3 models")
print("="*60)

import subprocess
subprocess.run([sys.executable, "main.py", "--compare-5", "--epochs", "150"], cwd=ROOT)
