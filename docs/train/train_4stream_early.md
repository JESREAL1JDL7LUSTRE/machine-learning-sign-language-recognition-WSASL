# train_4stream_early.py (4-Stream Early Fusion ST-GCN)

This document explains the 4-stream early-fusion training script and its data flow.
...
Early fusion means the model fuses these streams early inside the network.
...
Outputs:
- models/sign_stgcn_4stream_early.pth (best model weights)
- output/results_4stream_early.json (fold metrics, predictions, label map)
