# train_4stream_late.py code walk (4-stream, late fusion)

This is a code-focused walkthrough of the late-fusion 4-stream training script, mapped to the actual structure of the file.
...
`EARLY_FUSION = False` is the key difference for this file.
...
The model is built from the same multi-stream ST-GCN implementation, but `EARLY_FUSION = False` keeps streams separate longer and fuses later in the network. The fusion logic itself is in the model definition, not in this script.
